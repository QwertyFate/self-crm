...
async function toggleActivityComplete(activityId, completed) {
  try {
    ...
    // Refresh the day modal with fresh data
    const modal = document.getElementById('day-events-modal');
    if (modal) {
      // Re-fetch the day events reference and reopen the modal if it's still open
      const ev = calEvents.find(e => e.id === activityId);
      if (ev) {
        const fresh = await api.get('/api/calendar?start=' + ev.event_date + '&end=' + ev.event_date);
        // Update local calEvents for this item
        calEvents = calEvents.map(ce => ce.id === activityId ? { ...ce, completed } : ce);
        openDayModal(ev.event_date);
      }
    }
  } catch (e) {
