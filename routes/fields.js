...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM custom_fields WHERE workspace_id=$1 ORDER BY position ASC, id ASC',
      [req.workspaceId]
    );
    res.json(rows); // options is JSONB — already an array
  } catch (e) { next(e); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    const result = await pool.query(
      'DELETE FROM custom_fields WHERE id=$1 AND workspace_id=$2',
