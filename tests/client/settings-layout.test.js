...
describe('one card anatomy', () => {
  ...
  test('full-width cards use the wide class; no onboarding trigger card on this branch', () => {
...
describe('scripts and styles', () => {
  ...
  test('settings.js renders empty states and pipelines without inline spacing and picks the tab by role', () => {
    ...
    assert.ok((settings.match(/class="settings-empty"/g) || []).length >= 5);   // 7 on UIwithskill; the onboarding trigger card and the contact stages list are not on this branch
